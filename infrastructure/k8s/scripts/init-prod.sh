...
kubectl apply -f infrastructure/k8s/prod/deployment.yaml
kubectl apply -f infrastructure/k8s/prod/mariadb-configmap.yaml
kubectl apply -f infrastructure/k8s/prod/mariadb-deployment.yaml
kubectl apply -f "$TEMP_DIR/mariadb-secret.yaml"
kubectl apply -f infrastructure/k8s/prod/mariadb-service.yaml
...
if [[ "$TO_TERRAFORM" == "true" ]]; then
    ...
    echo "✅ Production Kubernetes resources initialized successfully!"
fi